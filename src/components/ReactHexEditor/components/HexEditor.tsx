...
import { byteToAscii, formatHex, formatHexByte, isMacLike } from "../utils";
...
import HexEditorContext, {
  HexEditorContextInterface,
} from "../contexts/HexEditorContext";
...
const reducer = (prevState: HexEditorState, mergeState: HexEditorAction) => ({
  ...prevState,
  ...mergeState,
});
...
const HexEditor: React.RefForwardingComponent<HexEditorHandle, HexEditorProps> =
  (
    ...
    const setSelectionRange = useCallback(
      (
        ...
        if (selectionStart == null) {
          ...
          selectionStart =
            prevSelectionDirection === SELECTION_DIRECTION_BACKWARD
              ? prevSelectionEnd
              : prevSelectionStart;
        }
        ...
        selectionEnd =
          selectionEnd == null
            ? selectionStart
            : Math.max(0, Math.min(selectionEnd, dataLength));
        ...
        for (let i = selectionStart; i < selectionEnd; i += 1) {
          selectedValue +=
            currentEditMode === EDIT_MODE_ASCII
    ...
    const setSelectionStart = useCallback(
      (offset: number, editMode?: EditModeType, _e?: React.MouseEvent) => {
        const { selectionAnchor } = stateRef.current;
    ...
    const setSelectionEnd = useCallback(
      (offset: number, editMode?: EditModeType, e?: React.MouseEvent) => {
        const {
    ...
    const handleKeyDown = useCallback(
      (e: React.KeyboardEvent<HTMLInputElement>) => {
        const { which, shiftKey, ctrlKey, metaKey } = e;
        ...
        switch (true) {
          ...
          case which === KEY_HOME: {
            ...
            if (shiftKey) {
              const cursorPosition =
                columns *
                Math.floor(Math.max(0, selectionAnchor - 1) / columns);
              ...
              const cursorPosition =
                columns * Math.floor(selectionAnchor / columns);
          ...
          case which === KEY_END: {
            ...
            if (shiftKey) {
              const cursorPosition =
                columns * (Math.floor(selectionAnchor / columns) + 1);
              if (selectionDirection === SELECTION_DIRECTION_BACKWARD) {
                setSelectionRange(selectionEnd, cursorPosition);
              } else {
                setSelectionRange(selectionStart, cursorPosition);
              ...
              const cursorPosition =
                columns * (Math.floor(selectionAnchor / columns) + 1) - 1;
      ...
      (e: React.ClipboardEvent<HTMLInputElement>) => {
        ...
        const values =
          currentEditMode === EDIT_MODE_ASCII
            ...
            : (
                clipboardText.replace(/[^0-9a-f]/gi, "").match(/.{2}/g) || []
              ).map((v) => parseInt(v, 16));
        ...
        setSelectionRange(
          Math.min(cursorOffset + values.length, dataLength - 1)
        );
      },
      [setValue, setSelectionRange]
    );
    ...
    useLayoutEffect(() => {
      if (rowListRef.current) {
        const { visibleStartIndex, visibleStopIndex } = stateRef.current;
    ...
    const formatOffset = useMemo(() => {
      const padToLength =
        2 * Math.ceil(formatHex(Math.max(0, data.length - 1)).length / 2);
    ...
    const { cursorColumn, cursorRow } = useMemo(() => {
      ...
      <HexEditorContext.Provider value={hexEditorContext}>
        <div
          className={joinClassNames(className, {
            ...
            [classNames.notFocused || ""]:
              !state.isFocused && state.selectionAnchor == null,
          })}
          style={editorStyle}
        >
          <input
