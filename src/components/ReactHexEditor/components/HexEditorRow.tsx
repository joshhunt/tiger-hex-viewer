...
export interface HexEditorRowProps {
  ...
  renderByte?: (props: RenderByteProps) => React.ReactNode;
}

function areRowPropsEquivalent(
  prevProps: HexEditorRowProps,
  nextProps: HexEditorRowProps
) {
  const {
  ...
  if (
    ...
    const prevHasSelection = hasSelection(
      prevOffset,
      prevOffsetEnd,
      prevSelectionStart,
      prevSelectionEnd
    );
    const nextHasSelection = hasSelection(
      nextOffset,
      nextOffsetEnd,
      nextSelectionStart,
      nextSelectionEnd
        ...
        <div className={classNames.byteValues} style={styles.byteValues}>
          {dataOffsets.map((offset, columnIndex) => {
            ...
            if (offset < data.length) {
              value =
                isCursor && nybbleHigh != null
                  ? (nybbleHigh << 4) | (0x0f & data[offset])
                  : data[offset];
            }

            const baseByteValueElement = (
              <HexByteValue
            ...
            const byteValue = renderByte
              ? renderByte({ offset, value, element: baseByteValueElement })
              : baseByteValueElement;
          ...
          <div className={classNames.asciiValues} style={styles.asciiValues}>
            {dataOffsets.map((offset, columnIndex) => {
              ...
              return (
                <HexAsciiValue
                  className={
                    isHeader ? classNames.asciiHeader : classNames.ascii
                  }
                  classNames={classNames}
                  columnIndex={columnIndex}
